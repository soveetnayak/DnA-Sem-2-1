...
def updatestoragedetails(con,cur):
    try:
        ...
        if choice == 1:
            ...
            query = "Update Storage SET Shelf_Number = %s where Book_id = %s" %(shelf_no,Book_id)
            print("Shelf_Number updated successfully")

        elif choice == 2:
            rack_no = input("Enter the new Rack_no: ")
            query = "Update Storage SET Rack_Number = %s where Book_id = %s" %(rack_no,Book_id)
            print("Rack_Number updated successfully")
